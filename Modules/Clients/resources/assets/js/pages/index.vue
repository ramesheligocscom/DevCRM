...
const fetchClients = async () => {
  ...
  const data = await $api(`/clients?search=${searchQuery.value ?? ""}&status=${statusFilter.value ?? ""
...
const deleteClient = async (id) => {
  try {
    ...
    const response = await $api(`/clients/${id}`, { method: "DELETE" });
...
const onClientAssignUpdate = async (item, value) => {
  try {
    const { data } = await $api(`/clients/${item.id}`, {
      method: "PUT",
...
const filterdCardData = async (status, index) => {
  if (selectedCard.value === index) {
    ...
    if (status === "Total Clients") {
      ...
      const data = await $api(`/clients?search=${searchQuery.value ?? ""
    ...
    <VCard class="mb-6">
      ...
      <div class="d-flex justify-lg-space-between" style="margin: 20px">
        ...
        <div class="d-flex gap-3">
          ...
          <VBtn v-if="$can('client','export-list')" @click="getClientExportList()" :loading="exportLoader" :disabled="exportLoader">
          ...
          <VBtn v-if="$can('client', 'create-client')" rounded="" icon="tabler-plus" @click="
        ...
        <template #item.name="{ item }">
          <v-tooltip location="top">
            <template v-slot:activator="{ props }">
              <RouterLink v-if="$can('client', 'show-client')" :to="`/admin/clients/view/${item.id}`"
