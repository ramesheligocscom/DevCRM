<template>
  <!-- 👉 Delete Dialog -->
  <VDialog max-width="500" :model-value="props.isDialogVisible" @update:model-value="updateModelValue" persistent
    scrollable>
...
const onConfirmation = async () => {
  ...
  try {
    const response = await $api(`${props.endpoint}`, { method: "DELETE", body: { delete_text: confirmationText.value ?? null, action: action.value ?? 'force_delete' }, })
